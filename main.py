...
class MainPage:
    ...
    def main_page(self):
        root = self.root
        root.title('Monkey测试工具,copyright @ 2023 by Shirley')
        ...
        # seed值
        Label(root, text='Seed序列:').place(x=10, y=45)
        # seed输入框
        self.seed = Entry(root, bd=5, bg='white')
        self.seed.place(x=90, y=45)

        # click控件
        Label(root, text='Click总数:').place(x=10, y=80)
        # click输入框
        self.click = Entry(root, bd=5, bg='white')
        self.click.place(x=90, y=80)

        # 间隔时间控件
        Label(root, text='间隔时间ms:').place(x=10, y=115)
        self.ms = Entry(root, bd=5, bg='white')
        self.ms.place(x=90, y=115)

        # 是否忽略崩溃
        Label(root, text='忽略崩溃:').place(x=10, y=160)
        # 下拉框，设置默认值为第一个字符串
        self.crash_is = ttk.Combobox(root, value=crash_is, state='readonly')
        self.crash_is.place(x=90, y=160)
